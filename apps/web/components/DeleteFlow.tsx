  ...
  async function requestOtp(e: React.FormEvent) {
    ...
    try {
      const res = await fetch('/api/notify/request', {
        ...
        body: JSON.stringify({ phone, purpose: 'delete_data' }),
      });
      const data = await res.json();
      if (!res.ok) {
        setError(data.error === 'invalid_phone' ? 'Invalid Indian mobile number.'
              : data.error === 'rate_limited' ? 'Too many requests. Wait a minute.'
              : data.error === 'too_many_otps' ? 'Too many OTPs for this number. Wait an hour.'
      ...
      <div className="bg-[var(--color-paper)] rounded-2xl p-7 max-w-[440px] w-full shadow-[0_20px_60px_-12px_rgba(15,31,95,0.5)]">
        <div className="flex justify-between items-baseline mb-1">
          <h2 className="text-xl font-bold tracking-tight">
            {step === 'phone' && 'Confirm your number'}
        ...
        {step === 'phone' && (
          <form onSubmit={requestOtp}>
            <p className="text-sm text-[var(--color-ink-dim)] mb-5">
              Enter the WhatsApp number you used. We&apos;ll send you a one-time code to confirm.
            </p>
        ...
        {step === 'otp' && (
          <form onSubmit={confirmDelete}>
            <p className="text-sm text-[var(--color-ink-dim)] mb-5">
              Sent code to <b className="font-mono text-[var(--color-ink)]">{maskedPhone}</b>. Once you confirm, your data is purged immediately.
            </p>
            ...
            {error && <div className="text-xs text-[var(--color-red)] font-semibold mb-3">{error}</div>}
            <div className="flex gap-2 justify-end">
              <button type="button" onClick={() => { setStep('phone'); setOtp(''); }} className="px-4 py-2 text-sm font-semibold text-[var(--color-ink-soft)] hover:text-[var(--color-ink)]">← Back</button>
        ...
        {step === 'done' && (
          <div className="text-center">
            ...
            <p className="text-sm text-[var(--color-ink-dim)] mb-6">
              ...
              Your phone number is no longer in our database.
            </p>
