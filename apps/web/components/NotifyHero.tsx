...
export function NotifyHero({ sites }: Props) {
  ...
  return (
    <section className="col center relative bg-[var(--color-bg)] flex flex-col overflow-hidden">
      <div className="max-w-[760px] mx-auto px-5 pt-10 pb-10 md:px-14 md:pt-20 md:pb-16 text-center relative z-10">
        <div className="inline-flex items-center gap-2.5 text-xs font-semibold text-[var(--color-ink-dim)] mb-5 md:mb-6">
        ...
        <h1 className="text-[clamp(30px,5vw,60px)] font-bold tracking-tight leading-[1.05] text-[var(--color-ink)]">
          Don't worry.<br />
        ...
        <p className="mt-4 md:mt-5 text-[15px] md:text-base font-medium text-[var(--color-ink-dim)] leading-snug max-w-[540px] mx-auto">
        ...
        <form className="notify-form mt-8 md:mt-10 bg-[var(--color-paper)] border border-[var(--color-border-strong)] rounded-2xl p-2 md:px-5.5 md:pl-5 md:py-2 flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-2.5 shadow-[0_8px_24px_-12px_rgba(15,31,95,0.15),_0_2px_6px_-2px_rgba(15,31,95,0.08)] text-left"
              onSubmit={(e) => { e.preventDefault(); setShowFlow(true); }}>
          <span className="hidden sm:inline text-base text-[var(--color-ink-dim)] font-medium whitespace-nowrap pl-3">Notify me when</span>
          <input
            ref={fieldRef}
            aria-label="Notify me when"
            className="flex-1 border-0 outline-0 bg-transparent text-[15px] md:text-base font-medium px-3 py-3 sm:py-3.5 min-w-0 text-[var(--color-ink)] placeholder:text-[var(--color-ink-faint)]"
            placeholder="the Aadhaar update portal starts working"
          />
          <button className="bg-[var(--color-blue)] text-white border-0 px-5 py-3 rounded-[9px] text-[13.5px] font-bold inline-flex items-center justify-center gap-2 transition-all hover:bg-[var(--color-blue-deep)] w-full sm:w-auto">
            Set alert
          </button>
        </form>

        <div className="mt-3 text-[11.5px] md:text-xs text-[var(--color-ink-faint)] flex items-center justify-center gap-x-3 gap-y-1.5 flex-wrap">
...
function ComingSoonMarquee() {
  ...
  return (
    <div className="relative z-10 mt-2 border-t border-[var(--color-border)]">
      <div className="max-w-[760px] mx-auto px-5 md:px-14 pt-6 md:pt-8 pb-4 md:pb-5 text-center">
      ...
      <div className="marquee relative overflow-hidden mx-4 md:mx-6 pb-6">
...
function DonateNudge() {
  return (
    <div className="relative z-10 border-t border-[var(--color-border)] bg-gradient-to-b from-transparent to-[var(--color-blue-soft)]/30">
      <div className="max-w-[640px] mx-auto px-5 md:px-14 py-8 md:py-10 text-center">
        ...
        <div className="mt-6 flex flex-col sm:flex-row items-stretch sm:items-center justify-center gap-2.5 sm:gap-3">
          <a
            href="/donate"
            className="inline-flex items-center justify-center gap-2 bg-[var(--color-blue)] text-white px-5 py-3 rounded-[10px] text-[13.5px] font-bold shadow-[0_2px_4px_rgba(15,31,95,0.18),_inset_0_1px_0_rgba(255,255,255,0.18)] hover:bg-[var(--color-blue-deep)] transition-all"
          >
            <span>₹</span>
            Support via UPI
          </a>
          <a
            href="/donate"
            className="inline-flex items-center justify-center gap-2 bg-[var(--color-paper)] border border-[var(--color-border-strong)] text-[var(--color-ink-soft)] px-5 py-3 rounded-[10px] text-[13.5px] font-bold hover:border-[var(--color-saffron)] hover:text-[var(--color-saffron)] hover:bg-[var(--color-saffron-soft)] transition-all"
          >
            <span>☕</span>
            Buy me chai
          </a>
